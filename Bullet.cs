...
public class Bullet : Photon.MonoBehaviour
{
    ...
    private void myMasterIs(int id, string launcherRef)
    {
        ...
        if (view == null) return; // Still gonna be null
        master = view.gameObject;
        switch (launcherRef)
        {
            case "hookRefL1":
                myRef = view.gameObject.GetComponent<HERO>().hookRefL1;
                break;
            case "hookRefL2":
                myRef = view.gameObject.GetComponent<HERO>().hookRefL2;
                break;
            case "hookRefR1":
                myRef = view.gameObject.GetComponent<HERO>().hookRefR1;
                break;
            case "hookRefR2":
                myRef = view.gameObject.GetComponent<HERO>().hookRefR2;
                break;
            default:
                Core.Log("Argument out of range @ Bullet.myMasterIs: " + launcherRef);
                throw new ArgumentOutOfRangeException(nameof(launcherRef));
        }
    }
    ...
    {
        this.nodes = new ArrayList {newPosition};
    }
    ...
    private void Start()
    {
        ...
        GameObject.Find("MultiplayerManager").GetComponent<FengGameManagerMKII>().RegisterHook(this);
    }
    ...
    {
        if (PhotonView.Find(id) != null) //BUG: NullReferenceException || Prob. fixed
