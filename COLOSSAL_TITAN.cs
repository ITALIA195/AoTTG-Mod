...
public class COLOSSAL_TITAN : Photon.MonoBehaviour
{
    ...
    public void blowPlayer(GameObject player, Transform neck)
    {
        ...
        {
            player.GetComponent<HERO>().blowAway((Vector3) ((vector.normalized * num) + (Vector3.up * 1f)), new PhotonMessageInfo());
        }
    ...
    public void Update()
    {
        ...
        {
            var gameObject1 = (GameObject)o;
            if (gameObject1.name.Contains("door")) //MOD: Remove
                Core.SendMessage(gameObject1.name);
        }
        Destroy(GameObject.Find("door_2"));
        Destroy(GameObject.Find("door_fine"));
    }
    ...
    { 
        ...
        if (door_broken == null)
        {
            ...
            Core.SendMessage("Door is still null!");
            return;
        }
    ...
    {
        GameObject.Find("MultiplayerManager").GetComponent<FengGameManagerMKII>().RegisterColossalTitan(this);
        if (this.myHero == null)
        {
    ...
    public void titanGetHit(int viewID, int speed)
    {
        ...
        if (view != null)
        {
            ...
            if ((vector.magnitude < this.lagMax) && (this.healthTime <= 0f))
            {
                ...
                {
                    GameObject.Find("MultiplayerManager").GetComponent<FengGameManagerMKII>().sendKillInfo(false, (string) view.owner.Name, true, "Colossal Titan's neck", speed);
