...
public class TITAN : Photon.MonoBehaviour
{
    ...
    public void DieByCannon(int viewID)
    {
        ...
        if (view != null)
        {
            ...
            if (this.nonAI)
            {
                FengGameManagerMKII.instance.titanGetKill(view.owner, damage, (string) PhotonNetwork.player.Name);
            }
            else
            {
    ...
    {
        if (grabTarget.GetComponent<HERO>() == Core.GetHero(PhotonNetwork.player.ID) && ModManager.Find("module.antigrab").Enabled)
            return;
    ...
    {
        if (IN_GAME_MAIN_CAMERA.isPausing && IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.SINGLE || IN_GAME_MAIN_CAMERA.gametype != GAMETYPE.SINGLE && !photonView.isMine) return;
    ...
    private int PunkNumber => GameObject.FindGameObjectsWithTag("titan").Count(obj2 => obj2.GetComponent<TITAN>() != null && obj2.GetComponent<TITAN>().name == "Punk");

    private void Grab(string key)
    {
        this.state = TitanState.grab;
        this.attacked = false;
        this.isAlarm = true;
        this.attackAnimation = key;
        this.crossFade("grab_" + key, 0.1f);
        ...
        if (!string.IsNullOrEmpty(key))
        {
            switch (key)
            {
                ...
                case "head_front_r":
                    ...
                    break;
                default:
                    Core.Log("ArgumentOutOfRangeException @ TITAN.Grab(string)", ErrorType.Warning);
                    break;
            }
        }
        if (this.isGrabHandLeft)
    ...
    {
        Core.SendMessage("Random debug text xd"); //TODO: Debug text
    ...
    private void justEatHero(GameObject target, Transform hand)
    {
        if (target != null)
        {
            ...
            {
                target.GetComponent<HERO>().DieRC(hand);
            }
        }
    }
    ...
    public void loadskin()
    {
        ...
        if (!(IN_GAME_MAIN_CAMERA.gametype != GAMETYPE.SINGLE && !base.photonView.isMine || (int) FengGameManagerMKII.settings[1] != 1))
        {
            ...
            if (IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.SINGLE)
            {
                base.StartCoroutine(this.LoadSkinTitan(body, eye));
            }
            else
            {
    ...
    public IEnumerator LoadSkinTitan(string body, string eye)
    {
        while (!this.hasSpawn)
        {
            yield return null;
    ...
    public void setAbnormalType(AbnormalType type, bool forceCrawler = false)
    {
        ...
        if (num == 4)
        {
            ...
            {
                if ((IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.SINGLE) && (PunkNumber >= 3))
                {
                    num = 1;
                }
    ...
    private void Start()
    {
        this.MultiplayerManager.RegisterTitan(this);
    ...
    public void suicide()
    {
        ...
        if (this.nonAI)
        {
            GameObject.Find("MultiplayerManager").GetComponent<FengGameManagerMKII>().sendKillInfo(false, string.Empty, true, PhotonNetwork.player.Name, 0);
        }
        ...
        if (view != null)
        {
            ...
            if (((vector.magnitude < this.lagMax) && !this.hasDie) && ((Time.time - this.healthTime) > 0.2f))
            {
                ...
                if (this.currentHealth < 0f)
                {
                    ...
                    if (this.nonAI)
                    {
                        FengGameManagerMKII.instance.titanGetKill(view.owner, speed, PhotonNetwork.player.Name);
                    }
                    else
                    {
    ...
    public void Update2() // Do not rename in "Update" so Photon wont call it every (?)
    {
                        ...
                        if (Vector3.Distance(this.baseTransform.position, this.targetCheckPt) < this.targetR)
                        {
                            if (this.checkPoints.Count > 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        //TODO: Decomment
                                        //Core.SendPublicMessage("<color=#A8FF24>*WARNING!* An abnormal titan is approaching the north gate!</color>");
                                    }
