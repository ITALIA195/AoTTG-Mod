...
public static class PhotonNetwork
{
    ...
    public static bool CloseConnection(PhotonPlayer kickPlayer)
    {
        ...
        return networkingPeer.OpRaiseEvent(203, null, true, new RaiseEventOptions { TargetActors = new[] { kickPlayer.ID } });
    }
    ...
    public static void DestroyPlayerObjects(int targetPlayerId)
    {
        if (VerifyCanUseNetwork())
        {
            ...
            {
                Core.LogFile("DestroyPlayerObject() filtered", ErrorType.Warning);
            }
            else
            {
    ...
    public static GameObject Instantiate(string prefabName, Vector3 position, Quaternion rotation, int[] targetActors)
    {
        if (connected && (!InstantiateInRoomOnly || inRoom))
        {
            ...
            if (networkingPeer.currentLevelPrefix > 0)
            {
                customEventContent[(byte)8] = networkingPeer.currentLevelPrefix;
            }
            customEventContent[(byte)6] = networkingPeer.ServerTimeInMilliSeconds;
            ...
            RaiseEvent(202, customEventContent, true, raiseEventOptions);
            Hashtable evData = customEventContent;
    ...
    {
        Core.ModManager.CallMethod("OnInstantiate");
    ...
    public static void UnAllocateViewID(int viewID)
    {
        ...
        {
            Debug.LogWarning(
                $"Unallocated manually used viewID: {viewID} but found it used still in a PhotonView: {networkingPeer.photonViewList[viewID]}");
        }
    }
    ...
    public static bool connected
    {
        get
        {
            ...
            return !networkingPeer.IsInitialConnect &&
                   networkingPeer.states != PeerStates.PeerCreated &&
                   networkingPeer.states != PeerStates.Disconnected &&
                   networkingPeer.states != PeerStates.Disconnecting &&
                   networkingPeer.states != PeerStates.ConnectingToNameServer;
        }
    }
    ...
    public static int FriendsListAge => ((networkingPeer == null) ? 0 : networkingPeer.FriendsListAge);
    ...
    public static bool inGame => inRoom || IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.SINGLE;
    ...
    public static PhotonPlayer player => networkingPeer?.mLocalActor;

    public static PhotonPlayer[] playerList => networkingPeer == null ? new PhotonPlayer[0] : networkingPeer.mPlayerListCopy;

    public static string playerName // FriendName
    {
        get
        {
